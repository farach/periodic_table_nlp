...
A raw count would reward long speeches. A rate per thousand alphabetic tokens
gives each speech a comparable scale, but a clean-looking line can still be
carried by one short document.
...
stopifnot(
  ...
  identical(trend_summary$max_rate_flag, c("ok", "ok", "under 500 tokens", "ok")),
  identical(trend_summary$survives_guard, c(TRUE, TRUE, FALSE, TRUE)),
  identical(as.numeric(length_context$median_alphabetic_tokens), 1958.5)
)
```

The denominator counts lowercase ASCII alphabetic tokens because the tokenizer
filters everything else before measuring speech length. The table says how often
each tracked word appears and where it was densest on that stated scale. Rates
from speeches under 500 alphabetic tokens are flagged because one occurrence in
such a document is at least 2 uses per thousand. The median speech here has about
1,959 alphabetic tokens, so the cutoff catches the smallest denominators without
removing most speeches.

## Plot the trajectories

A line chart earns its place here because the corpus spans 236 years. Each dot
is one speech, and each line connects speeches for one word. Open circles mark
rates from speeches under 500 alphabetic tokens.

```{r}
#| label: fig-word-trends
#| fig-cap: "Uses per thousand alphabetic tokens for four selected words across 60 inaugural addresses."
#| fig-alt: "Four small line charts, one per word, sharing a vertical scale: america, freedom, constitution, and children. Each panel draws its word in blue over faint grey lines for the other three words. America, freedom, and children reach their highest rates after 1990; constitution peaks in 1793. Rates from speeches under 500 alphabetic tokens are marked with open circles."
#| fig-height: 5
#| code-fold: true
...
# Without a `word` column, this copy is drawn in every panel as context.
context_lines <- plot_data |>
  rename(context_word = word)

ggplot(plot_data, aes(x = year, y = per_thousand_alphabetic)) +
  geom_line(
    data = context_lines,
    aes(group = context_word),
    colour = lesson_colours[["context"]],
    linewidth = 0.35
  ) +
  geom_line(colour = lesson_colours[["accent"]], linewidth = 0.7) +
  geom_point(
    aes(shape = below_min_tokens),
  ...
  facet_wrap(vars(word), ncol = 2) +
  labs(
    title = "Three of the four words reach their highest rate after 1990",
    subtitle = str_c(
      "Uses per 1,000 alphabetic tokens in 60 inaugural addresses.<br>",
      "Open circles mark speeches under 500 tokens; grey lines show the other words."
    ),
    x = "Speech year",
    y = NULL
  ) +
...
stopifnot(
  ...
  identical(as.numeric(round(max(plot_data$per_thousand_alphabetic), 2)), 12.35),
  identical(
    as.integer(plot_data$year[which.max(plot_data$per_thousand_alphabetic)]),
    2005L
  ),
  identical(sum(plot_data$below_min_tokens), 8L),
  identical(sum(trend_summary$max_year > 1990), 3L),
  identical(
    as.character(trend_summary$word[trend_summary$max_year < 1990]),
    "constitution"
  )
)
```
...
stopifnot(
  ...
  identical(as.integer(constitution_top_rates$total_alphabetic_tokens[1]), 135L),
  identical(as.numeric(constitution_top_rates$per_thousand_alphabetic[1]), 7.41),
...
The headline problem is visible for `constitution`. Its maximum rate is 7.41
per thousand alphabetic tokens in 1793, but that speech has only 135 alphabetic
tokens. One occurrence creates the peak. After removing that peak speech, the
next rate is 6.65 in 1861; with the denominator guard applied, the peak does not
survive because the reported maximum came from an under-500-token document.
Shuffling the 207 occurrences across fixed speech lengths gives a null mean of
5.31, a 90% interval from 3.42 to 7.43, and p = 0.233. The observed 1793 peak is
well within what this shuffle can produce.
...
- Normalize word counts per thousand alphabetic tokens before comparing
  speeches.
