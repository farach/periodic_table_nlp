...
- **Pseudonymization vs. Anonymization**: Replacing a name with `[PERSON]` is pseudonymization (the data might still be linked back to the person if other clues remain). Anonymization requires breaking those links so the subject cannot be reasonably identified.
...
- **Category-wise recall**: An honest evaluation measures recall (the percentage of true identifiers successfully masked) *per category*. A system might have 99% recall on phone numbers but only 60% on names, and an overall score hides that danger.
- **Legal basis and residual risk**: Processing text often involves separate legal boundaries, such as a lawful basis under GDPR or permitted uses under HIPAA. Teams maintain residual-risk documentation to describe the remaining chance of re-identification (the "reasonable basis to believe" standard) after masking.
