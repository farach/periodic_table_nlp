...
- **Pseudonymization vs. Anonymization**: Replacing a name with `[PERSON]` is pseudonymization (the data might still be linked back to the person if other clues remain). True anonymization makes re-identification impossible.
...
- **Legal basis and residual risk**: Processing and masking text requires documenting a legal basis (such as GDPR or HIPAA compliance) and maintaining residual-risk documentation that details the likelihood of re-identification from the leaked text that inevitably slips through.
