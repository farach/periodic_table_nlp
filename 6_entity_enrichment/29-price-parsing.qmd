...
stopifnot(
  identical(nrow(failure_results), 3L),
  is.na(failure_results$amount_lower[1]),
  identical(failure_results$amount_lower[2], 1.25),
  identical(failure_results$currency[3], "USD")
)
```

- **Value normalization**: The pattern misses `"thirty-eight thousand"` entirely because it only looks for digits.
- **Locale formats**: It reads the European format `1.250,50` incorrectly: it
  matches `1.250` without warning and yields an amount of 1.25 because it
  expects commas as thousands separators.
