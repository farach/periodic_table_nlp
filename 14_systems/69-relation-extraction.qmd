...
relation_schema <- tribble(
  ...
  "provider -> credential", "asserted, negated, hedged",
...
stopifnot(
  ...
  identical(sentences$text, sentence_text_copy),
...
place this lesson prints all 28 source sentences. It shows why most lines cannot
...
verbatim_inspection <- sentences |>
  mutate(
    linked_mentions = map_chr(
      ...
      \(id) {
        ...
        if (nrow(rows) == 0L) {
          ...
          paste(
            paste0(rows$mention, " (", rows$entity_id, ")"),
            collapse = "; "
          )
        }
      }
    ...
    can_make_schema_edge = "no complete same-sentence entity pair",
    page_subject_assumption = "not used"
  ) |>
  select(
    sentence_id,
    document_id,
    ...
    polarity,
    modality,
    can_make_schema_edge,
    page_subject_assumption
  )
...
knitr::kable(
  ...
  col.names = c(
    ...
    "Schema cue", "Polarity", "Modality", "Schema edge?",
    "Page subject assumption"
  ),
...
stopifnot(
  ...
  identical(
    verbatim_inspection$polarity[verbatim_inspection$sentence_id == "s002"],
    "negated"
  ),
  identical(
    verbatim_inspection$polarity[verbatim_inspection$sentence_id == "s027"],
    "negated"
  ),
  !any(str_detect(
    verbatim_inspection$can_make_schema_edge,
    regex("^yes", ignore_case = TRUE)
  )),
  all(verbatim_inspection$page_subject_assumption == "not used"),
...
relations between those mentions.
...
requirement is kept in the reference so the scoring table can show a miss.

```{r}
children <- parsed |>
  select(doc_id, token_id, child_id = token_id, child = token, child_lemma = lemma, child_dep = dep_rel, head_token_id)
...
argument_roles <- mention_heads |>
  left_join(
    parsed |>
      transmute(sentence_id = doc_id, head_token_id = token_id, dep_rel, token_head_id = head_token_id),
    by = c("sentence_id", "head_token_id")
  ) |>
  select(
...
conj_objects <- object_heads |>
  filter(object_dep == "conj") |>
  left_join(
    object_heads |>
      select(sentence_id, governing_object_head_id = object_head_id),
    by = c("sentence_id", "object_token_head_id" = "governing_object_head_id")
  )
...
status_for_trigger <- function(sentence_id, trigger_token_id, subject_head_id, object_head_id) {
  sentence_tokens <- parsed |>
    filter(doc_id == sentence_id)
...
extractor_cases <- reference_triples |>
  filter(sentence_id != "c13")

extracted_triples <- extractor_cases |>
  left_join(
    mention_heads |>
      select(
        sentence_id,
  ...
  mutate(
    evidence_start = pmin(subject_start, object_start),
    evidence_end = pmax(subject_end, object_end),
...
Tomas can now separate two tasks: finding candidate pairs for review and
asserting a schema triple only when the sentence supports it.
