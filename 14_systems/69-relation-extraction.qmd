...
The matcher behind the table looks for every alias in each sentence. It tries
longer aliases first, so `Riverton Skills Centre` is claimed as one organisation
before `Riverton` alone can match inside it, and it records the first and last
character of every match. An alias shared by two IDs is kept and marked
ambiguous rather than resolved by guessing.

```{r}
#| code-fold: true
#| code-summary: "Show the code that finds alias mentions"
...
relations between those mentions. The test-sentence IDs skip some numbers; no
test sentence was removed after the extractor ran.
...
`conj` a coordinated item. Lesson 18 introduces dependency parses.
...
```{r}
#| code-fold: true
#| code-summary: "Show the extractor code"
...
```{r}
#| code-fold: true
#| code-summary: "Show the pair and triple scoring code"
...
Many current pipelines ask a language model to return each triple as JSON that
must match a schema. A schema guarantee checks the format, not the content: a
reply can parse perfectly and still name the wrong entity or drop a negation.
Asking for the answer before any reasoning can also lower accuracy, so a
schema that includes a reasoning field should put it first. Keep this page's
evidence offsets and status column, and score the output against a reference
written before the model runs. The guide
[Using a language model in a reproducible analysis](../using-language-models.qmd)
covers the rest of the record-keeping.
...
- [Geng et al. (2025), JSONSchemaBench: a benchmark of structured outputs for language models (preprint)](https://arxiv.org/abs/2501.10868)
