...
right_action_em_zero <- score_table |>
  ...
  summarise(
    rows = paste(question_id, collapse = ", "),
    count = n(),
    reason = case_when(
      count == 0L ~ "no correctly handled answer row has exact match 0",
      count == 1L & all(partial_overlap) & all(acceptable_variant_in_answer) ~
        paste0(
          rows,
          " has exact match 0 because the answer contains an acceptable ",
          "variant but includes extra words"
        ),
      count > 1L & all(partial_overlap) & all(acceptable_variant_in_answer) ~
        paste0(
          rows,
          " have exact match 0 because each answer contains an acceptable ",
          "variant but includes extra words"
        ),
      TRUE ~
        paste0(rows, " have exact match 0 for wording differences")
    ),
    .groups = "drop"
  )
...
stopifnot(
  ...
  all(right_action_em_zero$count >= 0L),
  all(
    score_table |>
      filter(expected_action == "answer", action_correct, exact_match == 0) |>
      mutate(
  ...
  all(
    score_table |>
      ...
      pull(acceptable_variant_in_answer)
  )
)

em_zero_reason_text <- function(rows, count, partial_overlap = TRUE, variant_in_answer = TRUE) {
  case_when(
    count == 0L ~ "no correctly handled answer row has exact match 0",
    count == 1L && partial_overlap && variant_in_answer ~
      paste0(
        rows,
        " has exact match 0 because the answer contains an acceptable ",
        "variant but includes extra words"
    ...
    TRUE ~ paste0(rows, " have exact match 0 for wording differences")
  )
}

stopifnot(
  identical(
    em_zero_reason_text("", 0L),
    "no correctly handled answer row has exact match 0"
  ),
  identical(
    em_zero_reason_text("Q2, Q4", 2L),
    paste0(
