...
- parse a strict two-line model reply without repairing it;
...
kable(
  ...
  caption = "Input budgets asserted before each reader call",
  row.names = FALSE
)
```
...
Exact generated text can differ across machines, so the render asserts
structure, token caps, parsed actions, and screen columns rather than literal
wording.
...
**Exact match** is 1 only when the normalized prediction exactly equals one
...
class removes Unicode punctuation and symbols, not only ASCII punctuation. The
retrieval score above is cosine similarity between tf-idf vectors, so it ranges
from 0 to 1.
...
as a model comparison. In this render, `r right_action_em_zero$count` correctly
handled answer row has exact match 0 (`r right_action_em_zero$rows`), because
the generated answer includes extra words beyond the short acceptable variants.
