...
stopifnot(
  all(c(
    "dplyr", "huggingfaceR", "purrr", "readr",
...
```{r}
#| code-fold: true
#| code-summary: "Show the retrieval code"
...
Exact generated text can differ across machines, so every sentence that reports
a model result is computed from this render.
...
```{r}
#| code-fold: true
#| code-summary: "Show the scoring code"
...
```{r}
#| code-fold: true
#| code-summary: "Show the citation and support screens"
...
A common shortcut is to ask a second language model whether each answer is
supported by its cited passages. Recent benchmark work finds such judges close
to chance on hard unsupported answers, so their scores are a screening signal
at best. Keep mechanical screens like the ones above, and have a person read a
random sample of answers against their sources before reporting a support
rate.
...
- [Tamber et al. (2025), Benchmarking LLM faithfulness in RAG with evolving leaderboards](https://aclanthology.org/2025.emnlp-industry.54/)
