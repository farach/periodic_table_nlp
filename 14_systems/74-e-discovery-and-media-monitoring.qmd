...
The constructed prevalence is `r sprintf("%.0f%%", 100 * mean(review_records$reference_responsive))`, much higher than many legal-review matters. It was chosen so a small teaching page has enough responsive examples to inspect. Each record is a short note wrapped in one of 12 frames, such as "Reminder:" at the start or "Thanks." at the end, and every frame holds the same number of responsive records and the same number of non-responsive records. Before writing the file, the builder checked that surface features alone could not separate the two classes. It would have refused to write the file if any check reached an F1 of 0.60 or a balanced accuracy of 0.75. Both scores rise as a check separates the classes better: balanced accuracy, from lesson 44, averages the share of each class that a check gets right, and F1 combines the precision and recall defined in the keyword section below. The table below is the builder's own record of those checks.
