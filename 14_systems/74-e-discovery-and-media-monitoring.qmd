...
Grossman and Cormack report that in Blair and Maron's legal-search study, searchers believed they had found at least 75 percent of the relevant documents, while average recall was about 20 percent for the studied requests.

Ranked review is not the same as lesson 9's active learning. Active learning chooses uncertain examples to improve a model; ranked review chooses high-scoring unreviewed records so responsive material is found sooner.

The **elusion rate** is the share of unreviewed records that are responsive. A low elusion rate can still hide many misses. Suppose a review of 600 records from a 10,000-record collection finds 70 of its 100 responsive records. The 9,400 unreviewed records still hold 30 responsive records, so the elusion rate is 30 / 9,400, about 0.32 percent, while recall is 70 / 100, or 70 percent.
...
The constructed prevalence is `r sprintf("%.0f%%", 100 * mean(review_records$reference_responsive))`, much higher than many legal-review matters. It was chosen so a small teaching page has enough responsive examples to inspect.

Each record is a short note wrapped in one of 12 frames, such as "Reminder:" at the start or "Thanks." at the end; one frame leaves the note bare. Every frame holds the same number of responsive records and the same number of non-responsive records. Before writing the file, the builder tested whether the surface features it lists could separate the two classes. It would have refused to write the file if a check reached an F1 of 0.60 or a balanced accuracy of 0.75; the frame checks within keyword groups use only the balanced-accuracy limit.

Passing means that no listed check separated the classes that well, not that surface features carry no signal. Both scores rise as a check separates the classes better: balanced accuracy, from lesson 44, averages the share of each class that a check gets right, and F1 combines the precision and recall defined in the keyword section below. The table below is the builder's own record of those checks.
...
The table uses the builder's own shorthand:

- A **held-out** check learns a rule from some of the records and tests it on records it did not see.
- **Out-of-fold** scores come from cross-validation (lesson 43): each record is scored by a model that did not train on it. **Top 36** means the 36 highest scores, as many as there are responsive records.
- **Ridge** and **lasso** are two kinds of penalized regression, the same family as the ranker later on this page. **Snowball stop words** are a standard list of common words such as "the" and "to".
- The **core** is the note inside the frame, and its **opening** is its first word or first two words.
- A **rarity rule** flags records whose frame-and-first-word pair occurs at most k times, without using labels.
- **CK-D1** and **CK-D2** are the builder's names for its two frame checks. CK-D1 looks at each frame together with the first word of the note inside it, and CK-D2 compares the frames separately among records that contain a keyword and records that do not.

```{r}
#| include: false

stopifnot(
  identical(dplyr::n_distinct(review_metadata$shortcut_sweep), 1L),
  stringr::str_detect(review_metadata$shortcut_sweep[[1]], stringr::fixed("F1 >= 0.60")),
  stringr::str_detect(review_metadata$shortcut_sweep[[1]], stringr::fixed("balanced accuracy >= 0.75")),
  stringr::str_detect(review_metadata$shortcut_sweep[[1]], stringr::fixed("snowball")),
  stringr::str_detect(review_metadata$shortcut_sweep[[1]], stringr::fixed("ridge and lasso")),
  stringr::str_detect(review_metadata$shortcut_sweep[[1]], stringr::fixed("CK-D1")),
  stringr::str_detect(review_metadata$shortcut_sweep[[1]], stringr::fixed("CK-D2"))
)
```
...
```{r}
#| code-fold: true
#| code-summary: "Show the ranking code"
...
gain_alt_text <- paste0(
  ...
  "The line starts at zero and reaches ", review_progress$cumulative_responsive_found[[1]],
  " found at ", review_progress$cumulative_reviewed[[1]], " reviewed, then ",
  paste(
    paste0(
...
stopifnot(
  ...
  identical(sum(reviewed_records$reference_responsive), 27L),
  identical(nrow(unreviewed_records), 181L),
  identical(sum(unreviewed_records$reference_responsive), 9L),
  identical(nrow(missed_records), 9L),
...
gain_points <- review_progress |>
  add_row(
    stage = "start",
    responsive_found = 0L,
    cumulative_reviewed = 0L,
    cumulative_responsive_found = 0L,
    .before = 1
  ) |>
  mutate(
    point_label = case_when(
      stage == "start" ~ "",
      stage == "keyword seed" ~ str_c("keyword seed: ", responsive_found, " found"),
      TRUE ~ str_c(stage, ": +", responsive_found)
    )
  )
...
ggplot(
  ...
  geom_line(colour = lesson_colours[["accent"]], linewidth = 0.9) +
  geom_point(colour = lesson_colours[["accent"]], size = 2.8) +
  geom_text(
  ...
  scale_x_continuous(
    ...
    limits = c(0, max(review_progress$cumulative_reviewed) + 12)
  ) +
  scale_y_continuous(
  ...
  labs(
    title = "Returns shrink as review goes on: the ranked batches add 4, then 1",
    subtitle = "Responsive records found against records reviewed, as the reviewers see it",
    x = "Records reviewed",
    y = "Responsive records found"
  ) +
  theme_lesson(grid = "both")
```
...
stopifnot(
  ...
  identical(review_progress$cumulative_responsive_found, c(22L, 26L, 27L)),
  identical(review_progress$responsive_found, c(22L, 4L, 1L))
)
```
...
```{r}
#| code-fold: true
#| code-summary: "Show the monitoring code"
