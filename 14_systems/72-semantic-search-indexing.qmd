...
Hana looks back at the handbook question that failed in the question-answering lesson. A parent asked, "Where can my kid stay while I study at night?" The answer lived in passage H13, but the wording did not say `child care`.

This lesson uses a related search query, `somewhere for my kid during night class`. With the policy below, BM25 ranks H13 second because it shares only the function words `for` and `during`; the class-related distractor H15 shares `class`. That rank does not come from the meaning of `kid` or `night`. If stop words were removed, H13 would receive no lexical score for this query.
...
The example starts with H13 because it answers Hana's question. H15 is a distractor about laptops and class. A search method that always returns a top row can still put a distractor first.
...
The index terms are the units that BM25 sees. If a system needs exact course-code search, `dsc-104` must be a term, not an accident of two smaller terms.
...
The dense index uses the pinned `sentence-transformers/all-MiniLM-L6-v2` model. The local Hugging Face feature-extraction pipeline returns one vector per real token, including the `[CLS]` and `[SEP]` special tokens. The check below compares that count with the tokenizer's attention mask. There are no padding tokens in the single-text output, so the mean is mask-aware for this call; the lesson still asserts the mask length before pooling.

The model's own files say the sentence-transformer wrapper uses mean pooling and a normalization module. The model card says sentence-transformers truncates inputs longer than 256 word pieces by default and that training used sequences of at most 128 tokens. The page applies the model's pooling steps in R and refuses passages or queries over 256 word pieces before embedding.
...
batch_check_text <- handbook$text[1:2]
batch_output <- embedder$pipeline(as.list(batch_check_text))
batch_embeddings <- do.call(
  rbind,
  map(batch_output, mean_pool_l2)
)
single_embeddings <- do.call(
  rbind,
  map(batch_check_text, embed_one_text)
)
...
stopifnot(
  ...
  max(abs(batch_embeddings - single_embeddings)) < 1e-6,
...
All passages are short enough to embed without truncation. The batched and one-text calls agree for the first two passages, so the lesson embeds one text at a time for clarity and uses the batch check as a guard.

## Record stale-index checks

An index is tied to its passages and model conventions. The embedding dimension alone cannot detect a swap: all three small candidate models checked by the council had 384-dimensional vectors. The safer record includes model revision, pooling, prefixes, tokenizer behavior, library versions, and passage fingerprints.
...
index_record <- tibble(
  ...
  value = c(
    ...
    as.character(packageVersion("tokenizers"))
  )
)
...
top_hits <- bind_rows(
  bm25_ranks |> mutate(method = "BM25 lexical"),
  dense_ranks |> mutate(method = "dense"),
  rrf_ranks |> mutate(method = "RRF hybrid")
) |>
  filter(rank == 1L) |>
  left_join(
  ...
  select(method, query_id, query, passage_id, topic, score)
...
knitr::kable(
  ...
  format = "html",
  escape = TRUE,
  col.names = c("Method", "Query", "Query text", "Top passage", "Top topic", "Score"),
  caption = "Every method returns a top passage for every query",
  row.names = FALSE
)
```
...
knitr::kable(
  rank_evaluation |>
    mutate(
      across(
        c(recall_at_1, recall_at_3, reciprocal_rank),
        \(value) if_else(is.na(value), NA_character_, sprintf("%.3f", value))
      )
    ),
  col.names = c(
    "Method",
...
knitr::kable(
  paired_summary,
  col.names = c("Dense above BM25", "Ties", "BM25 above dense"),
  caption = "Paired reciprocal-rank counts for dense versus BM25 on the seven judged queries",
  row.names = FALSE
)
```
...
Hana keeps both lists in her review notes. The dense list rescued the child-care query, but the no-menu query showed why every search result still needs a relevance check.
