...
```{r}
#| code-fold: true
#| code-summary: "Show the stale-index checks"
...
```{r}
#| code-fold: true
#| code-summary: "Show the evaluation code"
...
## Choosing an embedding model now

The embedding model here is small and several years old. Newer open models, such as EmbeddingGemma and the Qwen3 embedding models, score higher on public benchmarks like MMTEB, and some can run on a laptop. A benchmark rank does not reliably predict performance on your own documents, and a new model means re-embedding the whole index, because vectors from different models cannot be compared. Judge any change on a small set of your own queries with relevance judgments written in advance, as this page does, and keep a lexical list beside the dense one.
...
- [EmbeddingGemma technical report](https://arxiv.org/abs/2509.20354)
